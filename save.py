...
def save(filename, allow_overwrite = True, context = g):
    def weave_data_line(we):
        shape_id1 = context.shapes.index(we.endpoints[0].s)
        shape_id2 = context.shapes.index(we.endpoints[1].s)
        line = f"{we.nwires} {we.incrs[0]} {we.incrs[1]} {we.color_key} "
        line += f"{shape_id1} {we.endpoints[0].i} {shape_id2} {we.endpoints[1].i}" 
        return line
    #
    if filename[-3:] != '.qw': filename += '.qw'
    open_flags = 'w' if allow_overwrite else 'x'
    #
    try:
        with open(filename, open_flags) as f:
            ...
            #
            p("COLORDATA")
            for k, color in context.colors.palette.items():
        ...
        with open(filename, 'r') as f:
            ...
            for i, line in enumerate(f):
                try:
                    ...
                    if line[-4:] == 'DATA': section = line; continue
                    #
                    match section:
                        case None: raise Exception()
                        case 'SHAPEDATA':
                            [shape_id, data] = line.split(':')
                            shaped_id = int(shape_id)
                            sh_dict[shape_id] = Shape.Create(data)
                        case 'COLORDATA':
                    ...
                    raise ParseError(i, line, section)
            ###
            weaves = []
            for i, line in weave_lines:
                try:
                    ...
                    raise ParseError(i, line, 'WEAVEDATA')
            ###
            loaded_subcontext = Rec(shapes = sh_dict.values(), weaves = weaves, palette = palette)
